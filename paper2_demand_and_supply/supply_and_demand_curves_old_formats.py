...
if calculate_material_supply_and_demand == True:
    ...
    for material in materials:
        df_all,fig = plot_demand_and_supply_curve(material)
...
### Create separate plots for MT and AC, per material.  
### Also create an inset of RING projected demand summary (Generation and storage, circuit breaker, transmission lines)
### So for example, each plot for Al has 4 lines: 1) MT scenario has 4 lines (RING demand for MT, USGS projected demand, DOE CMA projected demand, total domestic supply). 2) same but for AC.   

materials = ['aluminum']  # 'copper' or 'aluminum' 
all_rows = []
for material in materials:
    with open(f'supply_and_demand_{material}.pkl', 'rb') as f:
        dict_material = pickle.load(f)
...
## Read in the breakdown of the RING projected demand for the inset
sheets = ['ACGrid_2024MidCase_Al']
selected_material = 'aluminum'
df_tech = []
for sheet in sheets:
    df = pd.read_excel('Grid_RING_inputs_v2.xlsx', sheet_name=sheet, skiprows=6)
    df_filtered = df[df['Refined Materials'] == selected_material]
    df = df_filtered.copy()
    year_cols = [col for col in df.columns if str(col).isdigit()]

    df["tech_combined"] = np.select(
    [
        df["Tech"] == "Circuit breakers",
        df["Tech"] == "Transmission Lines"
    ],
    [
        "Circuit breakers",
        "Transmission"
